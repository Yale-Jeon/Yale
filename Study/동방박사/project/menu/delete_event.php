...
for ($i=0; $i < sizeof($list); $i++) { 
	$value1=$list[$i];
	$fore="DELETE FROM project.club_activity_registration WHERE club_activity_info_num=$value1";
	$sql = "DELETE FROM project.club_activity WHERE info_num=$value1";
	if ($conn->query($fore) === TRUE) {
		if ($conn->query($sql) === TRUE) {
			echo "<script language='Javascript'>alert('선택한 활동 삭제가 성공적으로 완료되었습니다!')</script>";
		}

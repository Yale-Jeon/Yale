...
class TravelingSalesmanProblem(GeneticAlgorithmProblem):
    ...
    def performEvolution(self, numIterations, numOffsprings, numPopulation, mutationFactor):
        ...
        factor = mutationFactor * len(self.dicLocations.keys())
        n = 0
        self.mut_rate = 1  # variation mutation rate
        self.sub_rate = 1  # variation substitution rate
        self.sel_rate = 1  # variation selection rate

        while True:
            currentTime = time.time()
            if (currentTime - startTime) >= self.time or n > numIterations:
                break
            offsprings = {}
            if numOffsprings + int(self.sub_rate) > numPopulation:
                self.sub_rate = numPopulation - numOffsprings
            parents_set = self.selectParents(population, numOffsprings + int(self.sub_rate) - 1, self.sel_rate)

            for itr in range(numOffsprings + int(self.sub_rate) - 1):
                offsprings[itr] = self.crossoverParents(parents_set[itr][0], parents_set[itr][1])
                for i in range(len(self.dicLocations.keys())):
                    x = random.random()
                    if x < mutationFactor:
                        self.mutation(offsprings[itr], factor * self.mut_rate)

            population = self.substitutePopulation(population, offsprings)
            
            mostFittest = self.findBestSolution(population)
            if self.best == mostFittest:
                self.mut_rate *= 1.2
                self.sub_rate *= 1.2
                self.sel_rate *= 1.2
            else:
            ...
            print('current Best =', self.calculateTotalDistance(self.best))
            if self.gui != '':
                self.gui.update()
            n += 1
    ...
    def selectParents(self, population, numOffsprings, sel_rate):
        rankCost = {}
        originalcost = {}
        BestCost = -999999
        WorstCost = 999999
        ...
        total = 0.0
        for itr in range(len(population)):
            rankCost[itr] = WorstCost - originalcost[itr] + (WorstCost - BestCost) / (size - 1) * sel_rate
